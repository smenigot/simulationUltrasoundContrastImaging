% [temps,pression] = En_solve_sps2d(med,iters,excit,attn,bubble)
%
...
% By Martin Anderson
%
% Modified by Sébastien Ménigot (Université François-Rabelais de Tours)
% sebastien.menigot@univ-tours.fr

function [temps,pression] = En_solve_sps2d(med,iters,excit,attn,bubble)

%% Bubble position
% L = size(bubble.position);
...
dt = med.dt;%med.stabf*min([med.dx med.dz])/max(med.c(:));
...
%% Save data and display pressure at specified interval

% % Display current pressure field with grayscale
%       % map scaled to set pressure of maximum possible 
%       % constructive interference at full white/black:
%       figure(1)
%       image_scale = 2*sum(max(excit.p0));
%       image(xax*1e2,zax*1e2,(p/image_scale)*32+32)
%       colormap(gray)
%       hold on
%       plot([min(xax*1e2) max(xax*1e2)], ...
%       	   med.dz*1e2*[1 1],'w:');
%       hold off
%       title_string = ['''Propagation after ' ...
% 		      num2str(round(cr*dt*1e7)/10) ' microseconds.'''];
%       eval(['title(' title_string ');']);
% %       axis image
%       axis xy
%       drawnow


% % if bubble.transducer
% % if rem(cr,4) == 0
% %     compteur = compteur + 1;
%     figure(75)
%     image_scale = 2*sum(max(excit.p0));
% %     surface(xax*1e3,zax*1e3,p,'linestyle','none')
%     image(xax*1e3,zax*1e3,(p/image_scale)*32+32)
%     colormap(gray)
% %     hold on
% %     plot3((position(1,:)-med.num_latpts/2).*med.dx*1e3,(position(2,:)-med.num_pml).*med.dz*1e3,ones(size(bubble.position(1,:))).*10e9,'r+')
% %     hold off 
%     title_string = ['''Propagation after ' num2str(round(cr*dt*1e7)/10) ' microseconds.'''];
%     eval(['title(' title_string ');']);
%     axis xy
%     xlabel('Axial dimension (mm)')
%     ylabel('Lateral dimension (mm)')
%     drawnow
% %     M(compteur)=getframe;
% % end
% % end
%% pression
    ...
    pression(cr,1) = p(8,8); % transducteur
    pression(cr,2) = p(229,8); % bulle
...
% M(fix(iters/4)+1)=getframe;
% movie(M)
% movie2avi( M, 'test.avi', 'fps', 10, 'quality', 100,'colormap','Jet','compression','indeo3') 
